...
from fastapi import FastAPI, HTTPException  # Web framework for APIs
...
import numpy as np                          # For NaN/Inf detection
...
# Config
# ----------------------------
S3_BUCKET = os.getenv("S3_BUCKET", "hemanth-regression-ml-project-2026")
REGION = os.getenv("AWS_REGION", "ap-south-1")
s3 = boto3.client("s3", region_name=REGION)

# Ensures the app always has the latest model/data locally, 
...
def predict_batch(data: List[dict]):
    ...
    if TRAIN_FEATURE_COLUMNS:
        df = df.reindex(columns=TRAIN_FEATURE_COLUMNS)

    # ── Catch NaN / Inf before they reach XGBoost ───────────────────────────
    nan_cols = df.columns[df.isnull().any()].tolist()
    inf_cols = df.columns[df.isin([np.inf, -np.inf]).any()].tolist()
    if nan_cols or inf_cols:
        raise HTTPException(
            status_code=422,
            detail=f"Bad input — NaN in: {nan_cols}, Inf in: {inf_cols}. "
                   "Likely caused by engineered/meta feature length mismatch."
        )
...
1. Imports (FastAPI, pandas, numpy, boto3, your inference function).
